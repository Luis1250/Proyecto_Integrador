...
static sf_message_subscriber_t Display_Thread_message_queue_0_0 = { .p_queue =
		&Display_Thread_message_queue, .instance_range =
		{ .start = 0, .end = 0 } };
...
void g_message_init(void) {
	tx_queue_create(&Display_Thread_message_queue,
			(CHAR *) "Display_Thread Message Queue", 1,
			&queue_memory_Display_Thread_message_queue,
			sizeof(queue_memory_Display_Thread_message_queue));
}
