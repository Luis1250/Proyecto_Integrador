...
{ .mode = TIMER_MODE_PERIODIC, .period = 100, .unit = TIMER_UNIT_PERIOD_MSEC, .duty_cycle = 50, .duty_cycle_unit =
  ...
  .channel = 2, .autostart = true, .p_callback = NULL, .p_context = &g_timer9, .p_extend = &g_timer9_extend, .irq_ipl =
          (BSP_IRQ_DISABLED), };
...
void IC_Thread_create(void)
{
    ...
    UINT err;
    err = tx_thread_create (&IC_Thread, (CHAR *) "IC_Thread", IC_Thread_func, (ULONG) NULL, &IC_Thread_stack, 1024, 1,
                            1, 1, TX_AUTO_START);
    if (TX_SUCCESS != err)
    {
