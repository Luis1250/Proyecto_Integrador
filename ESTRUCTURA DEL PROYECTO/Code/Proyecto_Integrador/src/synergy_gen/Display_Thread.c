...
static const external_irq_cfg_t g_external_irq_cfg = { .channel = 9, .trigger =
		EXTERNAL_IRQ_TRIG_FALLING, .filter_enable = true, .pclk_div =
		EXTERNAL_IRQ_PCLK_DIV_BY_64, .autostart = true, .p_callback = NULL,
		.p_context = &g_external_irq, .p_extend = NULL, .irq_ipl = (3), };
/* Instance structure to use this module. */
const external_irq_instance_t g_external_irq = { .p_ctrl = &g_external_irq_ctrl,
		.p_cfg = &g_external_irq_cfg, .p_api = &g_external_irq_on_icu };
...
const sf_external_irq_cfg_t g_sf_external_irq_cfg =
		{ .event = SF_EXTERNAL_IRQ_EVENT_SEMAPHORE_PUT, .p_lower_lvl_irq =
				&g_external_irq, };
/* Instance structure to use this module. */
const sf_external_irq_instance_t g_sf_external_irq = { .p_ctrl =
		&g_sf_external_irq_ctrl, .p_cfg = &g_sf_external_irq_cfg, .p_api =
...
const i2c_cfg_t g_i2c_cfg = { .channel = 2, .rate = I2C_RATE_STANDARD, .slave =
		0x48, .addr_mode = I2C_ADDR_MODE_7BIT, .sda_delay = 0,
#define SYNERGY_NOT_DEFINED (1)            
...
#endif
#undef SYNERGY_NOT_DEFINED	
		.p_callback = NULL, .p_context = (void *) &g_i2c, .rxi_ipl = (3),
		.txi_ipl = (3), .tei_ipl = (3), .eri_ipl = (3), .p_extend =
				&g_i2c_extend, };
/* Instance structure to use this module. */
const i2c_master_instance_t g_i2c = { .p_ctrl = &g_i2c_ctrl,
		.p_cfg = &g_i2c_cfg, .p_api = &g_i2c_master_on_riic };
...
const sf_touch_panel_i2c_cfg_t g_sf_touch_panel_i2c_cfg_extend = {
		.p_lower_lvl_i2c = &g_i2c, .p_lower_lvl_irq = &g_sf_external_irq, .pin =
				IOPORT_PORT_06_PIN_09, .p_chip =
				&g_sf_touch_panel_i2c_chip_sx8654, };
const sf_touch_panel_cfg_t g_sf_touch_panel_i2c_cfg = { .hsize_pixels = 240,
		.vsize_pixels = 320, .priority = 3, .update_hz = 10, .p_message =
				&g_sf_message0, .p_extend = &g_sf_touch_panel_i2c_cfg_extend,
		.event_class_instance = 0, .rotation_angle = 0 };
/* Instance structure to use this module. */
const sf_touch_panel_instance_t g_sf_touch_panel_i2c = { .p_ctrl =
		&g_sf_touch_panel_i2c_ctrl, .p_cfg = &g_sf_touch_panel_i2c_cfg, .p_api =
...
void sf_touch_panel_i2c_init0(void) {
	...
	ssp_err_g_sf_touch_panel_i2c = g_sf_touch_panel_i2c.p_api->open(
			g_sf_touch_panel_i2c.p_ctrl, g_sf_touch_panel_i2c.p_cfg);
	if (SSP_SUCCESS != ssp_err_g_sf_touch_panel_i2c) {
		g_sf_touch_panel_i2c_err_callback((void *) &g_sf_touch_panel_i2c,
				&ssp_err_g_sf_touch_panel_i2c);
	}
}
...
void Display_Thread_create(void) {
	...
	err_g_main_semaphore_lcdc = tx_semaphore_create(&g_main_semaphore_lcdc,
			(CHAR *) "Main Semaphore", 0);
	if (TX_SUCCESS != err_g_main_semaphore_lcdc) {
		tx_startup_err_callback(&g_main_semaphore_lcdc, 0);
	}

	UINT err;
	err = tx_thread_create(&Display_Thread, (CHAR *) "Display_Thread",
			Display_Thread_func, (ULONG) NULL, &Display_Thread_stack, 2048, 3,
			3, 20, TX_AUTO_START);
	if (TX_SUCCESS != err) {
